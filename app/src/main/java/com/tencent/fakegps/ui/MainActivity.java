...
//    private final double LON_DEFAULT = -122.401779;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBtnFlyTo = (Button) findViewById(R.id.btn_fly_to);
        mBtnFlyTo.setOnClickListener(this);
        updateBtnFlyTo();
    ...
    private void updateBtnFlyTo() {
        if (!JoyStickManager.get().isStarted()) {
            mBtnFlyTo.setEnabled(false);
        } else {
            mBtnFlyTo.setEnabled(true);
            if (JoyStickManager.get().isFlyMode()) {
                mBtnFlyTo.setText("Stop Fly");
            } else {
                mBtnFlyTo.setText("Fly");
            }
        }
    }
